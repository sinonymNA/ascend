...
// ── The Relay — Teacher host view (screen "relay_host") ─────────────────────
...
export default function RelayHost() {
  const { navigate, token } = useApp();
  const [phase, setPhase] = useState('setup'); // setup | lobby | claim | evidence | context | grading | results
  ...
  useEffect(() => {
    api.get('/api/write/games/relay/prompts').then((d) => setPrompts(d.prompts || [])).catch(() => {});
  }, []);
  ...
  const launch = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await api.post('/api/write/games/relay/create', { promptId: selectedPromptId });
      setRoomCode(res.roomCode);
      setPrompt(res.prompt);

      const socket = connectSocket(token);
      socketRef.current = socket;
      socket.emit('rl:host_join', { roomCode: res.roomCode });

      socket.on('rl:lobby_update', (data) => setPlayers(data.players || []));
      socket.on('rl:player_joined', (p) => {
        setPlayers((prev) => (prev.some((x) => x.id === p.id) ? prev : [...prev, p]));
      });
      socket.on('rl:teams_assigned', (data) => setTeams(data.teams || []));
      socket.on('rl:phase_claim', (data) => {
        setPhase('claim');
        setStageEndsAt(data.endsAt);
        setSubmittedCount(0);
      });
      socket.on('rl:phase_evidence', (data) => {
        setPhase('evidence');
        setStageEndsAt(data.endsAt);
        setSubmittedCount(0);
      });
      socket.on('rl:phase_context', (data) => {
        setPhase('context');
        setStageEndsAt(data.endsAt);
        setSubmittedCount(0);
      });
      socket.on('rl:submission_update', (data) => {
      ...
      socket.on('rl:error', (data) => setError(data.message));
  ...
  const startRelay = () => socketRef.current?.emit('rl:start_relay', { roomCode });
  const skipStage = () => socketRef.current?.emit('rl:skip_stage', { roomCode });
  const endGame = () => {
    socketRef.current?.emit('rl:end_game', { roomCode });
      ...
      <main style={{ maxWidth: 680, margin: '0 auto', padding: '22px 16px 80px', position: 'relative', zIndex: 3 }}>
        ...
        {phase === 'setup' && (
          <div>
            <p style={{ color: C.mid, fontSize: 13.5, lineHeight: 1.6, marginBottom: 18 }}>
              Pick a prompt — students are randomly split into teams of 3. Role 1
              writes the thesis, Role 2 sees it and writes evidence + reasoning,
              Role 3 sees the full paragraph and writes the contextualization.
              The compiled paragraph is graded as one LEQ paragraph, plus a cohesion bonus.
            </p>
        ...
        {phase === 'lobby' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            <div style={{ ...card, textAlign: 'center' }}>
              ...
              <div style={{ color: C.mid, fontSize: 12.5, marginTop: 8 }}>Students join The Relay with this code.</div>
            </div>
            <div style={{ ...card }}>
            ...
            <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} onClick={startRelay} disabled={players.length < 2} style={{ ...bigBtn, opacity: players.length < 2 ? 0.5 : 1 }}>
              Assign Teams &amp; Start
            </motion.button>
          </div>
        )}
        ...
        {(phase === 'claim' || phase === 'evidence' || phase === 'context') && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            <div style={{ ...card, textAlign: 'center' }}>
              <div style={{ fontSize: 11, color: C.muted, fontWeight: 800, letterSpacing: '0.1em', marginBottom: 6 }}>{STAGE_LABEL[phase]}</div>
              <div style={{ fontFamily: 'Cinzel, serif', fontSize: 40, fontWeight: 900, color: remaining <= 15 ? C.sunset : C.gold }}>
                {formatTime(remaining)}
              </div>
              <div style={{ color: C.mid, fontSize: 13, marginTop: 8 }}>{submittedCount} / {totalTeams || teams.length} teams submitted</div>
            </div>
            <div style={{ ...card }}>
              <div style={{ fontWeight: 800, color: C.text, fontSize: 13, marginBottom: 10 }}>Teams ({teams.length})</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {teams.map((t) => (
                  <div key={t.id} style={{ background: C.elevated, borderRadius: 8, padding: '8px 12px', display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {t.members.map((m) => (
                      <span key={m.id} style={{ color: C.mid, fontSize: 12, fontWeight: 700 }}>{m.name} <span style={{ color: C.gold, fontSize: 10.5, textTransform: 'uppercase' }}>({m.role})</span></span>
                    ))}
                  </div>
                ))}
              </div>
        ...
        {phase === 'grading' && (
          <div style={{ ...card, textAlign: 'center', padding: 40 }}>
            <div style={{ fontFamily: 'Cinzel, serif', fontWeight: 800, fontSize: 16, color: C.gold }}>Grading compiled paragraphs…</div>
          </div>
        )}
        ...
        {phase === 'results' && results && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ ...card, textAlign: 'center' }}>
              <div style={{ fontFamily: 'Cinzel, serif', fontWeight: 800, fontSize: 13, color: C.gold, letterSpacing: '0.2em', textTransform: 'uppercase' }}>Relay Complete</div>
            </div>
            {results.teams.map((t, i) => (
              <div key={t.id} style={{ ...card, border: `1px solid ${i === 0 ? C.pine + '70' : C.border}`, background: i === 0 ? `${C.pine}14` : C.card }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  ...
                  <span style={{ color: C.mid, fontSize: 12.5, fontWeight: 800 }}>{t.totalScore} pts ({t.score}/{t.maxScore} + {t.cohesionBonus} cohesion)</span>
                </div>
                <div style={{ color: C.text, fontSize: 13.5, lineHeight: 1.55, marginBottom: 8 }}>
                  {[t.claimText, t.evidenceText, t.contextText].filter(Boolean).join(' ')}
                </div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {t.members.map((m) => (
                    <span key={m.id} style={{ color: C.muted, fontSize: 11.5 }}>{m.name} ({m.role})</span>
                  ))}
                </div>
              </div>
            ))}
