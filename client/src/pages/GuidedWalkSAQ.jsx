...
// ── Highlighted prompt text for Phase 2 ───────────────────────────────────────
function HighlightedText({ text, elements, revealed }) {
  if (!text) return null;
  const marks = [];
...
// ── Ink-dot progress bar ───────────────────────────────────────────────────────
function ProgressDots({ phase }) {
  const idx = PHASES.indexOf(phase === 'complete' ? 'rubric' : phase);
  return (
...
function GwButton({ children, onClick, disabled, variant = 'primary', style = {} }) {
  const variants = {
...
// ── Phase shell — one focus object at a time, fade + 8px float reveal ─────────
function PhaseShell({ children, maxWidth = 640, reduceMotion }) {
